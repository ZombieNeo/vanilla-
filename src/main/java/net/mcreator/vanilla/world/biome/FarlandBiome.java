...
public class FarlandBiome extends VanillaElements.ModElement {
	@ObjectHolder("vanilla:farland")
	public static final CustomBiome biome = null;
	public FarlandBiome(VanillaElements instance) {
		super(instance, 1);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(1f).depth(2f).scale(2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.JUNGLE).waterColor(-12420880).waterFogColor(-12420880)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(),
							Blocks.STONE.getDefaultState(), Blocks.STONE.getDefaultState())));
			setRegistryName("farland");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Biome.createDecoratedFeature(Feature.DEFAULT_FLOWER,
					IFeatureConfig.NO_FEATURE_CONFIG, Placement.COUNT_HEIGHTMAP_32, new FrequencyConfig(8)));
			...
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(EntityType.PANDA, 15, 1, 5));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(EntityType.COD, 15, 1, 5));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(EntityType.PUFFERFISH, 15, 1, 5));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == Blocks.SPRUCE_LEAVES.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, Blocks.SPRUCE_LEAVES.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = position.getY() - 3 + height; genh <= position.getY() + height; genh++) {
							...
							for (int genx = position.getX() - k4; genx <= position.getX() + k4; genx++) {
								for (int genz = position.getZ() - k4; genz <= position.getZ() + k4; genz++) {
									...
									if (state.isIn(BlockTags.LEAVES) || state.getBlock() == Blocks.SPRUCE_LEAVES.getDefaultState().getBlock()) {
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.DARK_OAK_LOG.getDefaultState().getBlock()
					|| blockType == Blocks.SPRUCE_LEAVES.getDefaultState().getBlock() || blockType == Blocks.GRASS_BLOCK.getDefaultState().getBlock()
