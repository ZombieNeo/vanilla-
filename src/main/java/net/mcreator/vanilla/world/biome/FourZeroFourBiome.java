...
import net.mcreator.vanilla.block.CurruptionstoneBlock;
import net.mcreator.vanilla.block.CurruptionleafsBlock;
import net.mcreator.vanilla.block.CurruptiongrassBlock;
import net.mcreator.vanilla.block.CurrupionlogBlock;
...
public class FourZeroFourBiome extends VanillaElements.ModElement {
	@ObjectHolder("vanilla:fourzerofour")
	public static final CustomBiome biome = null;
	public FourZeroFourBiome(VanillaElements instance) {
		super(instance, 33);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-10086934).waterFogColor(-10086934)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(CurruptiongrassBlock.block.getDefaultState(),
							CurruptionstoneBlock.block.getDefaultState(), CurruptionstoneBlock.block.getDefaultState())));
			setRegistryName("fourzerofour");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == CurruptiongrassBlock.block.getDefaultState().getBlock()
							|| ground == CurruptionstoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == CurruptiongrassBlock.block.getDefaultState().getBlock()
									|| ground2 == CurruptionstoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), CurruptionstoneBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == CurruptionleafsBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, CurruptionleafsBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, CurrupionlogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == CurrupionlogBlock.block.getDefaultState().getBlock()
					|| blockType == CurruptionleafsBlock.block.getDefaultState().getBlock()
					|| blockType == CurruptiongrassBlock.block.getDefaultState().getBlock()
					|| blockType == CurruptionstoneBlock.block.getDefaultState().getBlock();
		}
